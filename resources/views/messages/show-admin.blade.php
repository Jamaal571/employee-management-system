...
<h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Admin</h2>
</div>
</x-slot>

<div class="py-8 px-4 lg:px-8 animate-fade-in" x-data="adminChatBox()" x-init="init()">
...
messages: [],
newBody: '',
pollUrl: '{{ route('messages.admin.poll') }}',
storeUrl: '{{ route('messages.admin.store') }}',
init() {
this.fetchMessages();
setInterval(() => this.fetchMessages(), 3000);
},
fetchMessages() {
fetch(this.pollUrl).then(res => res.json()).then(data => {
...
}).then(() => { this.fetchMessages(); this.$nextTick(() => this.scrollToBottom()); });
},
deleteMessage(id) {
