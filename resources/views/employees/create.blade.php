        ...
        <div class="max-w-lg mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" class="w-full border rounded p-2 mb-4 dark:bg-gray-700 dark:border-gray-600 dark:text-white">

                    <label class="block mb-1 font-medium dark:text-gray-200">Position</label>
                    <input type="text" name="position" value="{{ old('position') }}" class="w-full border rounded p-2 mb-4 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>

                    <label class="block mb-1 font-medium dark:text-gray-200">Salary</label>
                    <div class="flex gap-2 mb-4">
                        <input type="number" step="0.01" name="salary" value="{{ old('salary') }}" class="flex-1 border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                        <select name="currency" class="border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value="USD" {{ old('currency') == 'USD' ? 'selected' : '' }}>USD ($)</option>
                            <option value="SLSH" {{ old('currency') == 'SLSH' ? 'selected' : '' }}>SLSH</option>
                        </select>
                    </div>

                    <label class="block mb-1 font-medium dark:text-gray-200">Hire Date</label>
                    <input type="date" name="hire_date" value="{{ old('hire_date') }}" class="w-full border rounded p-2 mb-4 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
